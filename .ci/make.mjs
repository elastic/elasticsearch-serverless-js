...
async function codegen (args) {
  ...
  if (version === 'main') {
    await $`npm run serverless --prefix ${clientGeneratorPath} -- --version main`
  } else {
    await $`npm run serverless --prefix ${clientGeneratorPath} -- --version ${version.split('.').slice(0, 2).join('.')}`
  }
